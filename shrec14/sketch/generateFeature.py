...
outFeaDir = '/data/shrec14/SHREC14LSSTB_SKETCHES/SHREC14LSSTB_SKETCHES/feature'
## resized image ##
resize_H = 256
resize_W = 256
crop_H = 227
    ...
    if os.path.isdir(fullDir):
        ...
        for f in os.listdir(fullDir):
            ...
            if f.endswith('.png') or f.endswith('.jpg'):
                print('I am Here')
                img = caffe.io.load_image(inFile)
                import pdb; pdb.set_trace()
                img = transformer.preprocess('data', img)
                print('The image shape is ', img.shape)
                crop_img = img[:, int(resize_H/2) - int(crop_H/2): int(resize_H/2) + int(crop_H/2)+1, int(resize_W/2) - int(crop_W/2): int(resize_W/2) + int(crop_W/2)+1]
