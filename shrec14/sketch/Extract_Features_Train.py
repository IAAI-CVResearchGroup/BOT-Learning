# ======================================
# Filename: Extract_Features_Train.py
...
outFeaDir = '/data/shrec14/Sketch_Features_20Angles'
## resized image ##
resize_H = 256
resize_W = 256
crop_H = 227
    ...
    if os.path.isdir(fullDir):
        ...
        for f in os.listdir(fullDir):
            ...
            for ind in range(img_Num):
                ...
                im = Image.open(features_list[ind]).convert('RGB')
                for ind1 in range(Num_Angles):
                    im_rotate = im.rotate(18*ind1)
                    #im_rotate.show()
                    img = np.asarray(im_rotate)
                    ...
                    OutPath = outFile + '/' + img_Name[ind][:-4] + '_' + str(ind1).rjust(3,'0') + '.txt'
                    with open(OutPath, 'w') as newf:
                        np.savetxt(newf, net.blobs[feaLayer].data[0], fmt='%.4f', delimiter='\n')
# Extract_Features_Train.py ends here
# ====================================