...
class model(Dataset):
    ...
    def doubleContrastiveLoss(self):
        ...
        def contrastiveLoss(input_fea_1, input_fea_2, simLabel, margin, lossName):
            ...
            distance_contrastive = tf.add(tf.multiply(simLabel, distance_positive),
                                          tf.multiply(1. - simLabel, distance_negative))
        ...
        self.loss_sketch, self.loss_sketch_summary, _, _ = contrastiveLoss(self.sketch_1, self.sketch_2,
                                                                           self.simLabel_sketch, self.margin,
                                                                           'loss_sketch')

        # contrastive loss for shape
        self.loss_shape, self.loss_shape_summary, _, _ = contrastiveLoss(self.shape_1, self.shape_2,
                                                                         self.simLabel_shape, self.margin, 'loss_shape')

        # contrastive loss for sketch-shape 1
        self.loss_cross_1, self.loss_cross_summary_1, self.distance_cross_1, self.distance_positive_1 = contrastiveLoss(
            self.sketch_1, self.shape_1, self.simLabel_cross_1, self.margin, 'loss_cross_1')

        # contrastive loss for sketch-shape 2
        self.loss_cross_2, self.loss_cross_summary_2, self.distance_cross_2, self.distance_positive_2 = contrastiveLoss(
            self.sketch_2, self.shape_2, self.simLabel_cross_2, self.margin, 'loss_cross_2')
    ...
    def train(self):
        ...
        self.gm_optim = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=self.momentum).minimize(
            self.loss, var_list=self.gm_var_list)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
            ...
            for iter in range(self.maxiter):
                ...
                if self.lossType == 'contrastiveLoss':
                    ...
                    _, loss_, loss_sum_, sketch_fea, shape_fea, M_eval_1 = sess.run(
                        [self.gm_optim, self.loss, self.loss_summary, self.sketch_1, self.shape_1, self.groundMetric_1],
                        feed_dict={
                            self.input_sketch_fea_1: sketch_fea_1,
                            self.input_sketch_label_1: sketch_label_1,
                            self.input_sketch_fea_2: sketch_fea_2,
                ...
                cost.append(loss_)
                # ========================================
                # TODO Each Epoch output Loss and mAP One epoch = 1500 steps
                if iter == 2849:
                    ...
                    # print('Epoch: %i, mAP: %.4f, Loss: %.4f' %(row_num,mAP_,loss_sum_))
